...
void inc_no_lock(int &counter) {
    for (int i = 0; i < ITER; ++i) counter++; // data race (intentional)
}
...
int run_and_time(int T, F &&fn) {
    ...
    return static_cast<int>(std::chrono::duration<double, std::milli>(t1 - t0).count());
}
...
int main() {
    ...
    { // No lock (incorrect)
        int counter = 0;
        int ms = run_and_time(T, [&]{ inc_no_lock(counter); });
    ...
    { // Mutex
        int counter = 0;
        std::mutex m;
        int ms = run_and_time(T, [&]{ inc_with_mutex(counter, m); });
        std::cout << "[Mutex]   counter = " << counter
                  << " (expected " << expected << "), " << ms << " ms\n";
    }
    { // Atomic
        std::atomic<int> counter{0};
        int ms = run_and_time(T, [&]{ inc_atomic(counter); });
